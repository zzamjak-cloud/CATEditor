...
| 버튼 | 동작 |
|------|------|
| `sort` | 선택된 오브젝트(또는 그 직계 자식)를 이름 기준으로 정렬 |
| `rename` | 선택된 오브젝트 이름을 입력값으로 일괄 변경 |
| `replace` | 이름에서 왼쪽 입력값을 오른쪽 입력값으로 치환 |
| `prefix` | 이름 앞에 입력값 추가 |
| `suffix` | 이름 뒤에 입력값 추가 |
| `digit` | 선택 순서대로 번호 부여 (`이름_00`, `이름_01`, ...) |

- **정렬(`sort`)**: `_` 기준으로 분리된 다단계 정렬 적용 (숫자/문자 혼합 가능)
- **번호(`digit`)**: 오른쪽 한 자리 필드로 자릿수 지정 (0~9, 기본값 2)
- 입력/대체 필드는 GameObject가 선택된 동안에만 표시됨
- 모든 동작은 Undo 지원
